...
describe("Home routing", () => {
  it("routes project Git actions and integration overrides to WSL after navigating elsewhere", async () => {
    acceptProfile({
      profileKey: "win",
      homes: [home("win", nativeHost, "codex", "C:\\repo"), home("linux", ubuntu, "codex", "/repo")],
    });
    setHomeSelection(() => ({ projectPath: "C:\\repo" }));
    const stage = vi.spyOn(native, "gitStage").mockResolvedValue(null);
    const integration = vi.spyOn(native, "setIntegrationEnabled").mockResolvedValue({} as never);
    const project = localProjectPath(ubuntu, "/repo");
    await commands.gitStage(project, ["src/file.rs"]);
    await commands.setIntegrationEnabled("mcp", "server", false, project);
    expect(stage).toHaveBeenCalledWith("/repo", ["src/file.rs"], { homeKey: "linux" });
    expect(integration).toHaveBeenCalledWith("mcp", "server", false, "/repo", { homeKey: "linux" });
  });

  it("maps a WSL Git context back to local project paths", async () => {
    acceptProfile({ profileKey: "linux", homes: [home("linux", ubuntu, "codex", "/repo")] });
    vi.spyOn(native, "gitContext").mockResolvedValue({
      dir: "/repo",
      root: "/repo",
      commonDir: "/parent/.git",
      parentPath: "/parent",
    } as never);
    const result = await commands.gitContext(localProjectPath(ubuntu, "/repo"));
    expect(result.dir).toBe(localProjectPath(ubuntu, "/repo"));
    expect(result.parentPath).toBe(localProjectPath(ubuntu, "/parent"));
    expect(result.commonDir).toBe(localProjectPath(ubuntu, "/parent/.git"));
  });
